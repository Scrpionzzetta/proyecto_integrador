...
const obtenerReporte = async (req, res) => {
  ...
  try {
    ...
    if (especie) {
      const temporadasSnapshot = await db.collection('temporadas').get();
      const temporadasEspecie = new Set(
        temporadasSnapshot.docs
          .filter(doc => doc.data().fruta?.toLowerCase().includes(especie.toLowerCase()))
          .map(doc => doc.id)
      );
      recolecciones = recolecciones.filter(r => temporadasEspecie.has(r.temporadaId));
